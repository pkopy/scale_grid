...
function App() {
    ...
    const runSocket = () => {
        ...
        const socketText = new WebSocket(`ws://${host}:4101`);
        const socketTap = new WebSocket(`ws://${host}:4101`);


        socketTap.onopen = () => {
            setSocketTap(socketTap);
            console.log(socketTap);
            const sendM = () => {
    ...
    socketTap.onmessage = (e) => {
        ...
        console.log('responseTap', response);
    ...
    //     let data = e.data;
    //     const response = JSON.parse(data);
    //     console.log('responseAct', response)
    // }
    ...
    async function getAllImgs(socket, menuButtons, menuGuid) {
        ...
        if (menuButtons && menuButtons.length > 0) {
            ...
            for (let elem of menuButtons) {
                arrayOfmenu.push(elem.Name);
                if (!elem.img) {
                    console.log('app', elem.GUID);
                    for (let i = 0; i < 3; i++) {
    ...
    const tapParam = (param) => {
        if (nextClick) {
            ...
            if (start && socketTap.readyState === 1) {
                socketTap.send(JSON.stringify({COMMAND: 'TAP_PARAM', PARAM: param}))
            }
            setNextClick(false)
        }
...
//     const sendToSocket = () => {
//         console.log(socketText)
//         if (socketText.readyState === 1) {
//
//             socketText.send(JSON.stringify({ COMMAND: 'GET_MOD_INFO' }));
//             socketText.onmessage = (e) => {
//                 let data = e.data;
//                 const response = JSON.parse(data);
//                 // if (oldLabels !== labels) console.log('xman')
//                 console.log('froootor',response)
//
//                 if (response.RECORD) {
//                     // setLabels(response.RECORD.var_labels)
//                     setTextLabels(response.RECORD.var_labels)
//                 }
//             }}
//
//     }
//
//     const timer = setInterval(sendToSocket, 1000);
//     sendToSocket()
//     return () => {
//         clearInterval(timer)
//     }
