...
function App() {
    ...
    const [modIndex, setModeIndex] = useState(0);
    const [nameOfMod, setNameOfMod] = useState('');
    ...
    const runSocket = () => {
        ...
        socketMass.onopen = () => {
            ...
            function sendMass() {
                socketMass.send(JSON.stringify({COMMAND: 'GET_MOD_INFO'}));
    ...
    socketTap.onmessage = (e) => {
        ...
        if (response.COMMAND === 'EDIT_MESSAGE' && response.PARAM === 'SHOW_Text') {
            ...
            newOpenPanels[response.KEY] = 'inputPanel';
            setOpenPanels(newOpenPanels);
        ...
        if (response.COMMAND === 'EDIT_MESSAGE' && response.PARAM === 'SHOW_TEXT_MULTILINE') {
            ...
            const newOpenPanels = {...openPanels};
            setLayoutName('default');
            newOpenPanels[response.KEY] = 'inputPanelML';
            setOpenPanels(newOpenPanels);
            setInputPanel(response);
            response.RECORD.Password ? setKeyboardType('password') : setKeyboardType('textML')
        }

        if (response.COMMAND === 'EDIT_MESSAGE' && response.PARAM === 'SHOW_Int') {
            setOpenKeyboard(true);
            const newOpenPanels = {...openPanels};
        ...
        if (response.COMMAND === 'EDIT_MESSAGE' && response.PARAM === 'SHOW_Double') {
            setOpenKeyboard(true);
            const newOpenPanels = {...openPanels};
            setLayoutName('double');
            newOpenPanels[response.KEY] = 'inputPanel';
            setOpenPanels(newOpenPanels);
            setInputPanel(response);
            setKeyboardType('text');
        ...
        if (response.COMMAND === 'EDIT_MESSAGE' && response.PARAM === 'SHOW_DATE') {
    ...
    const pushButton = (param, key) => {
        if (nextClick) {
            if (start && socketTap.readyState === 1) {
                socketTap.send(JSON.stringify({COMMAND: 'PUSH_BUTTON', PARAM: param, KEY: key}))
            }
            setNextClick(false)
        }
        //prevents to fast click in the same element
...
//closing the actual menu
    const close = () => {
        ...
        // setOpenListPanel(false)
    };

    const showMenu = () => {
        setMenuButtons([]);
        if (start && socketTap.readyState === 1) {
            socketTap.send(JSON.stringify({COMMAND: 'EXECUTE_ACTION', PARAM: "actSetup"}))
    ...
    const setTabInTextPanel = (elem, index) => {
        if (!blocked) {
            ...
            for (let item of layout) {
                if (item.elem && item.elem.Name === 'TEXT') {
                    elem.elem.activeTab = index
                }
            }
