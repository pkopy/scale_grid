...
function App() {
    ...
    const [openEnumPanel, setOpenMenuPanel] = useState(false);
    ...
    socketTap.onmessage = (e) => {
        ...
        if (response.COMMAND === 'EDIT_MESSAGE' && response.PARAM === 'CLOSE') {
            ...
            if (response.KEY && openPanelsKeys.includes(response.KEY)) {
                // if (response.KEY === openPanels.enum) {
                //     setOpenMenuPanel(false);
                // }
                        ...
                        <div className="xx"
                            ...
                            <div
                                ...
                                <img src={`data:image/png;base64, ${elem.elem.img} `} draggable={false}
                                     onMouseDown={() => false} style={{pointerEvents: 'none', width: screen.imgWidth,transform: 'translate(0,12%)'}}
                                     alt='img'/>}
                            </div>}
